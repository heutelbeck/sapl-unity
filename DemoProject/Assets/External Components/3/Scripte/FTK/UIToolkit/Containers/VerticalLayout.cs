...
namespace FTK.UIToolkit.Containers
{
    ...
    public class VerticalLayout : LinearLayoutBase
    {
        [Min(0)]
        [SerializeField]
        private Vector2 size = new Vector2(0.05f, 0.25f);
        [SerializeField]
        ...
        public void UpdateLayout()
        {
            ...
            foreach (ElementDescriptor elem in elements)
                if (elem.IsValid)
                {
                    if (!elem.IsVoid)
                    {
                        (float extent, float offset) = GetExtentAndOffsetOf(currentIndex, numCells, elem.ProportionalSize, size.y, margin);
                        elem.ElementTransform.SetParent(transform, true);
                        elem.ElementTransform.localPosition = new Vector3(0, -offset, 0);
                        elem.Element.Size = new Vector2(size.x, extent);
    ...
    public class VerticalLayoutEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            VerticalLayout vl = (VerticalLayout)target;
            buttonStyle = new GUIStyle(GUI.skin.button);
            enableToggle = vl.IsRTEditing();
            if (GUILayout.Toggle(enableToggle, "RT editing", buttonStyle) != enableToggle)
            {
                enableToggle = !enableToggle;
                vl.SetEnableRTEditing(enableToggle);
            }
        }
